...
# метод для заполнения матрицы квадратами
...
# заполняем по определенным коордитнатам
fill(0, 4)
fill(3, 7)
fill(6, 10)
# обнуляем те значения где нет связей по варианту
ways[0][3] = 0
ways[3][0] = 0
# заполняем главную диагональ нулями
np.fill_diagonal(ways, 0)
# создаем матрицу заполненную нулями той же размерности
ways1 = np.zeros((n, n), int)
# делаем матрицу симметричную
for i in range(n):
    ways1 += np.diag(np.diagonal(ways, i), -i) + \
        np.diag(np.diagonal(ways, i), i)
print(ways1)
